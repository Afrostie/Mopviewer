...
public:
	...
	*/
	...
	/**
	*  the name of this particle (a std::string)
	*/
	std::string getName() {
		return this->name;
	}
	...
	*/
	...
	/**
	*  the score currently allocated to this particle
	*/
	double getScore() {
		return this->score;
	}
	...
	/**
	*  whether or not this particle has been set up as a spacecraft
	*/
	...
	*/
	MopItem exportAsMopItem() {
		...
		mi.z =this->z;
		mi.zd =this->zd;
        mi.totalForce =this->xf+this->yf+this->zf;
        mi.sliceNum=this->sliceNum;
        mi.scanLinePos = this->scanLinePos;
        mi.scanLine = this->scanLine;
		return mi;
	}
	...
	*/
	double findDistance(Particle &that) {
		float xdist(this->x - that.x_ext);
		float ydist(this->y - that.y_ext);
		float zdist(this->z - that.z_ext);
		float xt(maths.sqr(xdist));
		float yt(maths.sqr(ydist));
		float zt(maths.sqr(zdist));
		float distancesqr(xt+yt+zt);
	...
	*/
	...
	/*
	*   one entry of this particles history
	*/
	...
	/**
	*   the overall size of history array
	*/
	...
	/**
	*   the amount of the history array which is currently filled
	*/
	...
	*/
			...
			for (int i(0);i<numParticles;i++) {
				...
				if (i!=myIndex) { // no self comparisons
					...
					if (this->interactionPermission==Advisor::interactDifferentOnly) {
						...
						//we  here  in this particle has been set so as to not be allowed to interact with other chromosomes
						// so, is the other particle also a chromosome that I can't interact with?
					...
					if (allowed) {
						...
						float xdist(this->x - allParticles[i].x_ext);
						float ydist(this->y - allParticles[i].y_ext);
						float zdist(this->z - allParticles[i].z_ext);
						double xt(maths.sqr(xdist));
						double yt(maths.sqr(ydist));
						double zt(maths.sqr(zdist));
						float distancesqr(xt+yt+zt);
    ...
    void setupGravMeasuringParticle(double sliceDistance,double gapWidth,double gapLength) {
        this->mass = 1;
    ...
    void moveParticleToScanlineOrigin() {
        this->y=0;
        this->scanLine=0;
    }
    
    void moveParticleToNextScanline(int direction) {
        if (1) {
            this->x+=this->gapWidth;
            this->scanLine=0;
        }
        if (0) {
            this->x-=this->gapWidth;
    ...
    void moveParticleToSpecificLocation(float x, float y, float z, int sliceNum, int scanLine,int scanLinePos) {
            this->x=x;
            this->y=y;
            this->z=z;
            this->sliceNum = sliceNum;
            this->scanLine = scanLine;
            this->scanLinePos= scanLinePos;
    }

    
    void moveUpOneSlice() {
        this->x +=this->sliceDistance;
        this->sliceNum++;
        this->x=0;
    }
    
    void moveDownOneSlice() {
        this->x -=this->sliceDistance;
        this->sliceNum--;
        this->x=0;
    }
    
    void set_xf(double f) {
        this->xf = f;
    }
