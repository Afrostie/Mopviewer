...
void testWindow::init(std::string fileName, int skipCount) {
  ...
  glfwWindowHint(GLFW_RESIZABLE,
                 GL_FALSE);  // Stop the window from being resized
  ...
  // Create a vertex buffer object, as it is storing co-ordinates needs to be a
  // GL_ARRAY_BUFFER
  GLuint VBO, VAO, EBO;
  ...
  /*
   *  First parameter is which attribute to configure, n our shader we used
   *location 0, so 0
   *  Next value is the size of each vertex, in this case we are using 3D so 3
   *points
   *  The data is of type GLfloat
   *  GL_FALSE tells OpenGL not to normalise data as it is already normalised
   *  Next of the distance between each vertex in the array, for each vertex
   *we have 3 glfloats
  ...
  // Create a Program for our shaders. This will combine the current shaders for
  // output to screen
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Every rendering call after this will use the program, in this case
    // shaderProgram
...
void testWindow::key_callback(GLFWwindow* window, int key, int scancode,
                              int action, int mode) {
