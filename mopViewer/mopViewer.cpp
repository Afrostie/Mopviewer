...
int mopViewer::printOutput(MopState *mopstate, int currentRow, int currentitem,
                           WINDOW *window) {
...
void mopViewer::showStats(std::string fileName, int skipCount) {
  ...
  for (int i = 0; i < (2 * maxitems); i++) {
    ...
    if ((currentRow >= (8 * maxitems))) {
      currentRow2 =
          mopViewer::printOutput(mopstate, currentRow2, currentitem, window2);
      currentitem++;
    }
    // Else print to 1st column
    else if (currentitem >= 0) {
      currentRow =
          mopViewer::printOutput(mopstate, currentRow, currentitem, window1);
      currentitem++;
    }
  }
      ...
      case KEY_RIGHT:
        ...
        for (int i = 0; i < (2 * maxitems); i++) {
          if ((currentRow >= (8 * maxitems)) &&
            ...
            currentRow2 = mopViewer::printOutput(mopstate, currentRow2,
                                                 currentitem, window2);
            currentitem++;

          } else if (currentitem <= mopstate->getItemCount()) {
            currentRow = mopViewer::printOutput(mopstate, currentRow,
                                                currentitem, window1);
            currentitem++;
          }
        }
...
void mopViewer::selectGame(std::string fileName, int skipCount) {
  ...
  char welcome[] = "Enter 1 to View Stats, 2 to Open MopViewer and 3 to change Mopfile: ";
