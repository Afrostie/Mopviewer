...
private:
    ...
    /**
     * build a MopState from a fragment (Light Mop File)
     *
     */
    MopState  * buildMopStateFromFragmentForLighterMopFile(Fragment &source) {
        MopState *ms = new MopState();
        Fragment worker;
        Fragment thing;
        int numParticles;
...
public:
    ...
    MopState * readState(int option) {
        ...
        try {
            ...
            //std::cerr <<" > starting string to mopstate conversion" << std::endl;
             if (option== Advisor::standardMop) {
                  result = this->buildMopStateFromFragment(initial);

              } else {
                  result = buildMopStateFromFragmentForLighterMopFile(initial);
              }
     ...
     */
    MopState * readCyclingState(int option) {
        // get the next state from the mopfile
        MopState *incoming = this->readState(option);
        if (incoming == NULL) {
            this->resetFile();
            incoming = this->readState(option);
        }
        return incoming;
    } 
    ...
    void writeStatelight (Particle *localSet, int environmentSetSize) {
        std::stringstream tmp;
        MopItem mi;
        tmp << environmentSetSize;
        for (int x(0); x<environmentSetSize; x++) {
    ...
    /**
     * This writes the current set of particles to the mopfile.
     * ait can be used either for generating lighter mop files, or for the grav mapping build.
     */    void writeMopItemToSmallerMapFile(MopItem &mi,bool gravMap) {
        std::stringstream tmp;
         if(gravMap) {
            tmp << "|";
            tmp << mi.x << ",";
